...
const TimelinePage = _ref => {
  ...
  const _onPressBackgroundHandler = (type, event) => {
    ...
    const dateIsoString = convertPositionToISOString(event.nativeEvent.locationX, event.nativeEvent.locationY, startDate, timeIntervalHeight.value, columnWidth, tzOffset);
    switch (type) {
      case 'longPress':
